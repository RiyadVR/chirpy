...
const getRefreshTokenRow = `-- name: GetRefreshTokenRow :one
SELECT token, created_at, updated_at, user_id, expires_at, revoked_at FROM refresh_tokens WHERE token = $1
`

func (q *Queries) GetRefreshTokenRow(ctx context.Context, token string) (RefreshToken, error) {
	row := q.db.QueryRowContext(ctx, getRefreshTokenRow, token)
	var i RefreshToken
	err := row.Scan(
		&i.Token,
		&i.CreatedAt,
